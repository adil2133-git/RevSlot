...
export const disputeResolvedTemplate = (params: DisputeResolvedParams) => {
  ...
  if (recipientRole === "advisor") {
    if (isRefunded) {
      bodyText = `Your dispute for session <strong>${eventTypeName}</strong> (Booking #${bookingId}) has been <strong>approved</strong>. A 100% refund has been processed back to your original payment method via Razorpay (typically reflects in 5-7 business days).`;
    } else {
      bodyText = `Your dispute for session <strong>${eventTypeName}</strong> (Booking #${bookingId}) has been <strong>reviewed and dismissed</strong> following review of meeting attendance records.`;
    }
  } else {
    // Reviewer
    if (isRefunded) {
      bodyText = `The dispute for session <strong>${eventTypeName}</strong> (Booking #${bookingId}) has been resolved with a full refund granted to the client. Escrow funds for this session have been cancelled.`;
    } else {
      bodyText = `The dispute for session <strong>${eventTypeName}</strong> (Booking #${bookingId}) has been <strong>dismissed</strong>. Meeting logs confirmed your attendance. Your escrow payment has been released to your available balance.`;
    }
  }
